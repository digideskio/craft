	...
	craft.app = {
		...
		onFeaturedSpeakerClick: function (e) {
			var href = $(e.target).attr('href'),
				id = href.replace(/#(speakers|workshops)\//g, ''),
			...
			this.openSpeaker(href.indexOf('workshops') !== -1 ? 'workshops' : 'speakers', id, true);
		},
		...
		init : function () {
			...
			carousel.find('.controls').on('click', function (e) {
				current += $(this).hasClass('right') ? 1 : -1;
				...
				carousel.find('.slider').css({
					transform: 'translateX(-' + (carousel.find('img').eq(current)[0].offsetLeft) + 'px)'
				});

				if (current === 0) {
					carousel.find('.controls.left').addClass('hidden');
				} else {
					carousel.find('.controls.left').removeClass('hidden');
				}

				if (current === all - 1) {
					carousel.find('.controls.right').addClass('hidden');
				} else {
					carousel.find('.controls.right').removeClass('hidden');
				}
			});
